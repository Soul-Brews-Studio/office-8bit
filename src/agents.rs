...
fn sync_agents(
    ...
    // Wait for rooms to be created before placing agents
    if office.rooms.is_empty() { return; }
    ...
    // Track per-session desk index to spread agents across desks
    let mut session_desk_idx: Vec<(String, usize)> = Vec::new();
    ...
    for (idx, data) in registry.agents.iter().enumerate() {
        ...
        if let Some((entity, _)) = existing_entity {
            ...
            // Get this agent's desk index within its session
            let session_offset = if let Some((_, count)) = session_desk_idx.iter_mut().find(|(s, _)| s == &data.session) {
                let c = *count;
                *count += 1;
                c
            } else {
                session_desk_idx.push((data.session.clone(), 1));
                0
            };

            let (room_idx, desk_pos) = find_desk_for_agent(&office, &data.session, session_offset);
...
/// Find best room and desk for an agent.
/// `session_offset` = how many agents from same session already placed.
fn find_desk_for_agent(office: &OfficeMap, session: &str, session_offset: usize) -> (usize, (i32, i32)) {
    // Match room by session name (case-insensitive)
    let room_idx = office.rooms.iter()
        .position(|r| r.name.to_lowercase() == session.to_lowercase())
        .unwrap_or(session_offset % office.rooms.len());

    let room = &office.rooms[room_idx];
    let desk_idx = session_offset % room.desks.len().max(1);
